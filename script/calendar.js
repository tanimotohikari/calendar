    ...
    for (let w = 0; w < 6; w++) {
      ...
      for (let d = 0; d < 7; d++) {
        if (w == 0 && d < startDay) {
          ...
          calendarHtml += `<td class='is-disabled'>${ num }</td>`;
          dayCount++;
        } else if (thisDay + 2 > dayCount && thisMonth === currentMonth) {
          // 本日より前の日程は選択できないようにする
          calendarHtml += `<td class='is-disabled'>${dayCount}</td>`;
          dayCount++;
        } else {
          if (thisDay + 2 === dayCount && thisMonth === currentMonth && !selectDateStatus) {
            calendarHtml += `<td class='is-selected' data-month='${ currentMonth }' data-year='${ currentYear }'>${ dayCount }</td>`;
          } else {
